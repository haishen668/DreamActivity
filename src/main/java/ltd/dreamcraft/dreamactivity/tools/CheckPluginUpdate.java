    ...
    public static String checkPluginUpdate(String apiUrl) {
        ...
        try {
            ...
            try {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    ...
                    if ("DreamActivity".equals(name)) {
                        ...
                        if (isNeedUpdate(DreamActivity.in().getDescription().getVersion(), latestVersion)) {
                            ...
                            getConsoleSender().sendMessage("§b|> §aDreamActivity§e插件已经是§c最新版本§e无需更新。");
                            // 如果不需要更新就返回字符串
                            return "NOTREQUIRED";
                        }
                ...
                DreamActivity.in().getLogger().severe("解析API返回的JSON数据时发生错误：" + e.getMessage());
                return "ERROR";
            }
        } catch (Exception e) {
            DreamActivity.in().getLogger().severe("更新DreamActivity插件时发生错误：" + e.getMessage());
            return "ERROR";
        }
    }
